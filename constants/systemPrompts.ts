...
export const SYSTEM_PROMPTS = {
  ...
  - 마크다운 코드 블록(\`\`\`), 설명, 사족, 주석을 절대 포함하지 마세요.
  - JSON 문자열 내에 이스케이프되지 않은 따옴표(")를 사용하지 마세요. 필요하면 작은따옴표(')를 사용하세요.
  - 배열이나 객체의 마지막 항목 뒤에 쉼표(trailing comma)를 넣지 마세요.
        ...
        "subsections": [
          {
            "sub_number": 1,
            "title": "소제목",
            "detail": "이 소제목에서 다룰 구체적 사건, 예시, 논리 전개 방향"
          }
        ]
      }
    ]
  ...
  5. **코드 블록 금지:** 원고 본문에는 \`\`\` 사용 금지.
  6. Markdown 형식을 사용하되, 최상위 제목(#)은 쓰지 마세요. 소제목은 ###를 사용하세요.
  7. **용어 통일:** 핵심 키워드는 반드시 원래 용어를 그대로 사용하고, 동의어나 다른 표현으로 바꾸지 마세요.
  8. **강조 표기 남발 금지:** 본문에서 \`**\`, \`__\`, \`*\`, \`_\` 같은 마크다운 강조 표기는 사용하지 마세요.
     - 강조가 필요하면: 따옴표(" ") 또는 괄호, 혹은 문장 구조로 강조하세요.
  9. **사실 정확성:** 숫자, 연도, 통계, 고유명사 등 사실적 정보는 정확하게 작성하세요. 불확실하면 "약", "대략", "~경향이 있다" 등으로 표현을 완화하세요.
  `,
