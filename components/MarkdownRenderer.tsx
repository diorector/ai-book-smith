...
export default function MarkdownRenderer({ text, theme }: MarkdownRendererProps) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (headerMatch) {
            ...
            if (level === 1) {
                elements.push(<h1 key={i} className="font-display">{parseInline(content)}</h1>);
            } else if (level === 2) {
                elements.push(<h2 key={i} className="font-display">{parseInline(content)}</h2>);
            } else if (level === 3) {
                elements.push(<h3 key={i} className="flex items-center gap-2 font-display"><span className="text-[var(--ink-faint)]">§</span>{parseInline(content)}</h3>);
            } else {
                elements.push(<h4 key={i}>{parseInline(content)}</h4>);
            }
            continue;
        }
        ...
        elements.push(<p key={i}>{parseInline(cleanLine)}</p>);
    }
